...
const ONBOARDING_STEPS: OnboardingStep[] = [
  {
    ...
    subtitle: 'Registra y analiza tus transacciones de forma simple',
    icon: 'wallet',
  },
  {
    id: 'track',
    title: 'Escanea recibos',
    subtitle: 'Captura gastos automáticamente con tu cámara',
    icon: 'camera',
  },
  {
    id: 'analyze',
    title: 'Ve tus patrones',
    subtitle: 'Entiende mejor en qué gastas tu dinero',
    icon: 'analytics',
  },
];
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      <View className="flex-row items-center justify-between px-6 pt-4">
        ...
        {!isLastStep && (
          <TouchableOpacity
            ...
            <Text className="font-medium text-slate-600">Saltar</Text>
          </TouchableOpacity>
        )}
      </View>
      ...
      <Animated.View
        ...
        <View className="mb-12 h-32 w-32 items-center justify-center rounded-full bg-slate-200">
          <View className="h-24 w-24 items-center justify-center rounded-full bg-slate-300">
            <Ionicons name={currentStepData.icon} size={40} color="#64748b" />
          </View>
        </View>

        {/* Text Content */}
        <View className="mb-16 items-center">
          <Text className="mb-4 text-center text-3xl font-bold leading-tight text-slate-800">
            {currentStepData.title}
          </Text>
          <Text className="max-w-xs text-center text-lg leading-relaxed text-slate-600">
      ...
      <View className="px-8 pb-12">
        <TouchableOpacity
          onPress={handleNext}
          className="flex-row items-center justify-center rounded-2xl bg-slate-800 px-8 py-4">
